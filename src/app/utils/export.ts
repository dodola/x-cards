...
export const tweet2NotionBlock = (tweet: XConfig) => {
  const { username, time, text, likes, shares, replies, video, images } = tweet
        ...
        rich_text: [
          {
            type: "text",
            text: {
              content: `Posted on ${formattedDate}`
            }
          }
        ]
      }
...
export const tweet2Markdown = (tweetData: XConfig[]) => {
  const mdArray = tweetData.map((tweet) => {
    const { username, time, text, likes, shares, replies, video, images } =
      tweet
  ...
  try {
    ...
    if (images && images.length > 0) {
      for (let i = 0; i < images.length; i++) {
        ...
        if (!response.ok) {
          console.warn(
            `Failed to fetch high quality image, falling back to original URL for image ${i + 1}`
          )
          // 如果高清版获取失败，尝试原始URL
    ...
    const zipName =
      "" || `twwet_assets_${username || "none"}_${text.slice(0, 15)}`
    // 使用 file-saver 保存文件
